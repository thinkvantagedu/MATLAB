...
canti.generatePmSpaceSingleDim(structSwitch, drawRow, drawCol);

% generate damping coefficient space, the combination is stiffness then damping.
canti.generateDampingSpace(damLeng, damBond);
...
% approaches.
canti.rbInitial(nPhiInitial, reductionRatio, ratioSwitch, 'original', 1);
...
while canti.err.max.val > canti.err.lowBond
    ...
    canti.extractMaxErrorInfo('original', randomSwitch, 1); % greedy + 1
    disp({'Greedy iteration no' canti.countGreedy})
    
    canti.extractMaxPmInfo('original', 1);    
    canti.greedyInfoDisplay('original');
    ...
    canti.rbEnrichment(nPhiEnrich, reductionRatio, ratioSwitch, 'original', 1);
    canti.reducedMatricesStatic;
    canti.reducedMatricesDynamic;
