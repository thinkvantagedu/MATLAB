...
[node, ~] = ABAQUSReadINPGeo(INPfilename);
...
Nphi.trial = 1;
ERR.store_lag_lin = zeros(length(MTX_M.fre.OR.all), 1);
ERR.log.store_lag_lin = zeros(length(MTX_M.fre.OR.all), 1);
sigma.store_lag_lin = [];
...
while ERR.val>1e-1
    ...
    ERR.store_lag_lin(Nphi.trial) = ERR.store_lag_lin(Nphi.trial)+ERR.val;
    %     ERR.log.store_lag_lin(Nphi.trial) = ERR.log.store_lag_lin(Nphi.trial)+ERR.log.val;
...
% keyboard
sigma.store_lag_lin=[sigma.store_lag_lin; nonzeros(sigma.val)];
Nphi.trial=Nphi.trial-1;
%%
...
err.max.store_lag_lin=[];
err.max.log_store=[];
err.loc.store_lag_lin=[];
err.store_lag_lin = zeros(domain.length.I1, domain.length.I2);
err.log.store_lag_lin = zeros(domain.length.I1, domain.length.I2);
...
for i_trial = 1:size(pm.space.comb, 1)
    ...
    err.store_lag_lin(i_trial) = err.store_lag_lin(i_trial)+err.val;
    err.log.store_lag_lin(i_trial) = err.log.store_lag_lin(i_trial)+log10(err.val);
...
toc
%%
[err.max.val, err.loc.idx.max]=max(err.store_lag_lin(:));
err.max.store_lag_lin=[err.max.store_lag_lin; err.max.val];
...
for i_test = 1:test_NO
    ...
    ERR.iter.store_lag_lin = Dis.fre.OR.otpt.iter.exact-Dis.fre.OR.otpt.all.appr;
    Nphi.iter = 2;
    [phi.fre.ERR, ~, sigma.val] = SVD(ERR.iter.store_lag_lin, Nphi.iter);
    sigma.store_lag_lin=[sigma.store_lag_lin; nonzeros(sigma.val)];
    ...
    for i_iter = 1:size(pm.space.comb, 1)
        ...
        err.store_lag_lin(i_iter) = err.store_lag_lin(i_iter)+err.val;
        err.log.store_lag_lin(i_iter) = err.log.store_lag_lin(i_iter)+log10(err.val);
    ...
    pm.iter.row=pm.space.comb(err.loc.idx.max, :);
    err.loc.val.max=pm.iter.row(:, 1:2);
    err.loc.store_lag_lin=[err.loc.store_lag_lin; err.loc.val.max];
...
% find phi.fre.all (RB for MP).
Nphi.trial = 1;
ERR.store_lag_qd = zeros(length(MTX_M.fre.OR.all), 1);
ERR.log.store_lag_qd = zeros(length(MTX_M.fre.OR.all), 1);
sigma.store_lag_qd = [];
...
while ERR.val>1e-1
    ...
    ERR.val = abs((norm(Dis.fre.OR.otpt.trial.exact-Dis.fre.OR.otpt.trial.svd, 'fro'))/...
        norm(Dis.fre.OR.otpt.trial.exact, 'fro'));
    %     ERR.log.val = log10(ERR.val);
    ERR.store_lag_qd(Nphi.trial) = ERR.store_lag_qd(Nphi.trial)+ERR.val;
    %     ERR.log.store_lag_qd(Nphi.trial) = ERR.log.store_lag_qd(Nphi.trial)+ERR.log.val;
    if Nphi.trial >= length(MTX_M.fre.OR.all)/2
        warning('size of phi exceed half of DOF number');
    elseif Nphi.trial >= length(MTX_M.fre.OR.all)
        error('size of phi exceed DOF number')
...
% keyboard
sigma.store_lag_qd=[sigma.store_lag_qd; nonzeros(sigma.val)];
Nphi.trial=Nphi.trial-1;
%%
% error response surface for MP.
MTX_K.fre.RE.I11_I20_IS0 = phi.fre.all'*MTX_K.fre.OR.I11_I20_IS0*phi.fre.all;
...
err.max.store_lag_qd=[];
err.max.log_store=[];
err.loc.store_lag_qd=[];
err.store_lag_qd = zeros(domain.length.I1, domain.length.I2);
err.log.store_lag_qd = zeros(domain.length.I1, domain.length.I2);
h = waitbar(0,'Please wait when iterations being processed...');
steps=size(pm.space.comb, 1);
% each iteration computes one norm error.
for i_trial = 1:size(pm.space.comb, 1)
    ...
    err.val = norm(Dis.fre.OR.otpt.trial.res, 'fro')/norm(Dis.fre.OR.otpt.trial.exact, 'fro');
    err.store_lag_qd(i_trial) = err.store_lag_qd(i_trial)+err.val;
    err.log.store_lag_qd(i_trial) = err.log.store_lag_qd(i_trial)+log10(err.val);
...
toc
%%
[err.max.val, err.loc.idx.max]=max(err.store_lag_qd(:));
err.max.store_lag_qd=[err.max.store_lag_qd; err.max.val];
err.max.log_store=[err.max.log_store log10(err.max.val)];
pm.iter.row=pm.space.comb(err.loc.idx.max, :);
err.loc.val.max=pm.iter.row(:, 1:2);
%%
...
for i_test = 1:test_NO
    ...
    %%
    % ERROR = current exact solution - previous approximation.
    ERR.iter.store_lag_qd = Dis.fre.OR.otpt.iter.exact-Dis.fre.OR.otpt.all.appr;
    Nphi.iter = 2;
    [phi.fre.ERR, ~, sigma.val] = SVD(ERR.iter.store_lag_qd, Nphi.iter);
    sigma.store_lag_qd=[sigma.store_lag_qd; nonzeros(sigma.val)];
    phi.fre.all = [phi.fre.all phi.fre.ERR];
    phi.fre.all = GramSchmidtNew(phi.fre.all);
    ...
    for i_iter = 1:size(pm.space.comb, 1)
        ...
        err.val = norm(Dis.fre.OR.otpt.trial.res, 'fro')/norm(Dis.fre.OR.otpt.iter.exact, 'fro');
        err.store_lag_qd(i_iter) = err.store_lag_qd(i_iter)+err.val;
        err.log.store_lag_qd(i_iter) = err.log.store_lag_qd(i_iter)+log10(err.val);
    ...
    pm.iter.row=pm.space.comb(err.loc.idx.max, :);
    err.loc.val.max=pm.iter.row(:, 1:2);
    err.loc.store_lag_qd=[err.loc.store_lag_qd; err.loc.val.max];
    MTX_K.fre.RE.all.appr = MTX_K.fre.RE.I11_I20_IS0*pm.space.comb(err.loc.idx.max, 3)+...
        MTX_K.fre.RE.I10_I21_IS0*pm.space.comb(err.loc.idx.max, 4)+...
        MTX_K.fre.RE.I10_I20_IS1*0.01;
    [Dis.fre.RE.otpt.all.appr, Vel.fre.RE.otpt.all.appr, Acc.fre.RE.otpt.all.appr, ...
...
hold on
suptitle('estimation of error in error (refined grid)');
subplot(2, 3, test_NO)
surf(linspace(domain.bond.L.I1, domain.bond.R.I1, domain.length.I1), ...
    linspace(domain.bond.L.I2, domain.bond.R.I2, domain.length.I2), err.store_lag_lin'-err.store_lag_qd');
xlabel('parameter 1', 'FontSize', 10)
ylabel('parameter 2', 'FontSize', 10)
zlabel('estimation of e in e', 'FontSize', 10)
set(gca,'fontsize',10)
axis([-1 1 -1 1])
axi.lim = [-3*10^-3 10^-3];
zlim(axi.lim)
