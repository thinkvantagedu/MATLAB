...
while err.match > err.bd
    ...
    resp.store.all_pm.hhat = cell(no.pre.hhat, no.rb, no.phy, no.t_step);
            ...
            for i_phy = 1:no.phy
                ...
                for i_ts = 1:no.t_step
                    if i_ts == 1
                        ...
                        resp.otpt.zeros = sparse(no.dof, i_ts-2);
                        resp.otpt.nonzeros = ...
                            resp.store.order.all_pm{2}(:, 1:no.t_step-i_ts+2);
                        resp.otpt.asemb = [resp.otpt.zeros resp.otpt.nonzeros];
                        resp.store.all_pm.hhat(i_pre, i_rb, i_phy, i_ts) = ...
                            {resp.otpt.asemb(:)};
                    end
    ...
    err.pre.trans_store.hhat = cell(no.pre.hhat, 2);
    ...
    for i_pre = 1:no.pre.hhat
        ...
        err.pre.trans_store.hhat(i_pre, 1) = {i_pre};
        resp.pre.store.hhat = resp.store.all_pm.hhat(i_pre, :, :, :);
        % cat scan from the first cell index to the last, in this case it's
        % rb to phy to time. 
        resp.pre.col_asemb.hhat = sparse(cat(2, resp.pre.store.hhat{:}));
        
        resp.pre.col_asemb.hhat = ...
            [resp.store.fce.hhat{i_pre}(:) -resp.pre.col_asemb.hhat];
        err.pre.trans.hhat = resp.pre.col_asemb.hhat'*resp.pre.col_asemb.hhat;
        % err.pre.trans.hhat is sparse symmetric, thus save half and sparse
        % it. 
        
        err.pre.trans.hhat = {sparse(triu(err.pre.trans.hhat))};
        err.pre.trans_store.hhat(i_pre, 2) = err.pre.trans.hhat;
...
% % % % %         clear err.otpt
        ...
        % \alpha. Requires exact solution in pm domain, only for test
        % purpose. 
    ...
    if refinement.cond <= refinement.thres
        ...
        if turnon == 1
            ...
            GSAPlotFigure(font_size.label, font_size.axis, ...
                ...
                err.max.val0, err.max.val_exactwRB, ...
                err.store.exactwRB.val, cool, ...
...
if recordon == 1
    ...
    '/GreedyAppro/L2NormIterAdapt/t[%.1f]l[%.2f]rb0[%g]',...
